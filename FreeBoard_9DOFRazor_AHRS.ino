 ...
 *
/*
Compile this file and upload the binarry file to 
9 Degrees of Freedom - Razor IMU - https://www.sparkfun.com/products/10736

Created by gemi254@hotmail.com 07/04/104

#send #FBF to return to razor mode and #on to begin calibration
...
/*
 ...
 "#of" - Go to freeboard mode. This is the default mode, type #FBF to exit this mode and calibrate with #on
 // Sensor data output
...
#define OUTPUT__MODE_FREEBOARD 5 // Outputs nmea and freeboard data to serial port
// Output format definitions (do not change)
#define OUTPUT__FORMAT_TEXT 0 // Outputs data as text
#define OUTPUT__FORMAT_BINARY 1 // Outputs data as binary float

// Select your startup output mode and format here!#
int output_mode =  OUTPUT__MODE_FREEBOARD; //OUTPUT__MODE_ANGLES;
...
*/
#define ACCEL_X_MIN ((float) -474)
#define ACCEL_X_MAX ((float) 158)
#define ACCEL_Y_MIN ((float) -465)
#define ACCEL_Y_MAX ((float) 163)
#define ACCEL_Z_MIN ((float) 228)
...
#define MAGN_X_MAX ((float) 705)
#define MAGN_Y_MIN ((float) -694)
#define MAGN_Y_MAX ((float) 644)
#define MAGN_Z_MIN ((float) -504)
#define MAGN_Z_MAX ((float) 734)
...
#define CALIBRATION__MAGN_USE_EXTENDED true
const float magn_ellipsoid_center[3] = {74.3855, -6.79154, -15.5620};
const float magn_ellipsoid_transform[3][3] = {{0.847536, 0.0388958, -0.0285492}, {0.0388958, 0.861570, 0.0337472}, {-0.0285492, 0.0337472, 0.989204}};
...
*/
#define GYRO_AVERAGE_OFFSET_X ((float) 40.17)
#define GYRO_AVERAGE_OFFSET_Y ((float) 122.92)
#define GYRO_AVERAGE_OFFSET_Z ((float) 4.16)
...
#endif
  //Setup the freeboard mode
  freeboard_setup();
}
void serialEvent() {
    if( output_mode != OUTPUT__MODE_FREEBOARD) return;      
...
// Time to read the sensors again?
  if((millis() - timestamp) >= OUTPUT__DATA_INTERVAL)
  {
    ...
    else if (output_mode == OUTPUT__MODE_ANGLES || output_mode == OUTPUT__MODE_FREEBOARD )  // Output angles or freeboard mode
    {
...
{
  //Freeboard mode
  if( output_mode == OUTPUT__MODE_FREEBOARD){        
  ...
  if (Serial.available() >= 2)
  {
    if (Serial.read() == '#') // Start of new control message
    {
      ...
      else if (command == 'o') // Set _o_utput mode
      {
        ...
        else if (output_param == 'f') // Freeboard mode
        {        
          output_mode = OUTPUT__MODE_FREEBOARD;        
          turn_output_stream_off();
          reset_calibration_session_flag = true;
        }
