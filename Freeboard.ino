/* This file is part of the freeboard modification */
...
void printPitchRoll(){
	...
	str.print("$YXXDR,A,");
        //pitch
        char pBuf[7];
        dtostrf( TO_DEG(pitch) , 3, 1, pBuf);
	str.print(pBuf); //prints 1 decimal, but round to degree, should be good enough
	str.print(",D,PTCH,A,");
	//roll
        char rBuf[7];
        dtostrf( TO_DEG(roll) , 3, 1, rBuf);
        str.print(rBuf);
        str.print(",D,ROLL,A,");
	//yaw
        char yBuf[7];
        dtostrf( TO_DEG(yaw) , 3, 1, yBuf);
        str.print(yBuf);
        str.print(",D,YAW*");
...
void updateROT(){
  //executed every 0.2 secs
     //value is deg/s
     //float rot=(*0.06097);
     float rot=(yaw*0.06097);
     rotList.addValue(-degrees(rot));
     //rotList.addValue(-TO_DEG(rot));
